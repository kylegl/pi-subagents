...
• Skill routing: pass task-specific skills through the execution skill parameter for the child to load; do not load them in the parent. Parent-only orchestration/supervision skills remain in the parent and must not be forwarded to ordinary children.
...
• CHAIN: { chain: [{agent:"agent-a",skill?}, {parallel:[{agent:"agent-b",count:3,skill?}]}], skill? } - sequential pipeline with optional parallel fan-out
...
• Skill routing: pass task-specific skills through execution skill for the child to load; do not load them in the parent. Parent-only orchestration/supervision skills remain in the parent and must not be forwarded to ordinary children.
...
• SINGLE {agent, task?, skill?}; PARALLEL {tasks:[{agent,task,skill?,count?,output?,reads?,progress?}], concurrency?, worktree?}; CHAIN {chain:[{agent,task?,skill?},{parallel:[...]}], skill?}.
• Pass task-specific skills through execution skill for the child to load; do not load them in the parent. Parent-only orchestration/supervision skills remain in the parent and must not be forwarded to ordinary children.
